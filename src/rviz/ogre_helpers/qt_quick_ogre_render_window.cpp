...
void QtQuickOgreRenderWindow::onWindowChanged(QQuickWindow* window)
{
  ...
  connect(this, &QtQuickOgreRenderWindow::ogreInitialized,
          [this]() { Ogre::Root::getSingleton().addFrameListener(this); });
...
void QtQuickOgreRenderWindow::updateFBO()
{
  ...
  QSGGeometry::updateTexturedRectGeometry(&geometry_, QRectF(0, 0, size_.width(), size_.height()),
                                          QRectF(0, 0, 1, 1));
...
void QtQuickOgreRenderWindow::initializeOgre()
{
  ...
  connect(window(), &QQuickWindow::beforeRendering, this, &QtQuickOgreRenderWindow::render,
          Qt::DirectConnection);
